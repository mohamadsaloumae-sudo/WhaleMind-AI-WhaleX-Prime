    ...
    while True:
        try:
            ...
            try:
                ...
                log.info("🔎 Predator: فُحص %d | أعلى %.1f | بلا داتا %d | ميت %d | تلاعب %d | نقاط<6 %d | بلا مفتاحية %d | منطقة %d | BTC %d | ثقة %d | حارس %d | MTF %d | صدر %d",
                         _ps["scanned"], _ps["best_score"], _ps["no_data"], _ps["dead_vol"], _ps["spoof"],
                         _ps["low_score"], _ps["no_key"], _ps["zone"], _ps["btc"], _ps["low_conf"],
                         _ps["guardian"], _ps["mtf"], _ps["emitted"])
            except Exception:
                pass
...
async def _meme_live_guard():
    while True:
        try:
            ...
            # 🛑 راديوم مُطفأ: صفر بِرَك جديدة في يومين (الإطلاقات في pump.fun)
            #    وكان يزاحم حارس السحب على اتصال Helius فيُرفض الاثنان (HTTP 429).
            #    الاتصال مُنح للحارس اللحظي الذي يمنع -1,085% (17 كارثة).
            await live_loop()
        except Exception as _e:
            import logging as _lg
