...
async def get_price(symbol: str) -> Optional[float]:
    ...
    if _mx:
        ...
        import time as _tt
        # ⚡ البثّ اللحظي أولاً — WebSocket يكتب السعر في الذاكرة (0.000s)
        #    كان fetch_ticker لكل عملة = 11 ثانية لـ27 صفقة.
        try:
            from radars.multi.price_stream import get_price as _ws_px
            _wp = _ws_px(_mx[1])
            if _wp and _wp > 0:
                return float(_wp)
        except Exception:
...
PEAK_GIVEBACK = 3.0
PEAK_MIN_KEEP = 0.65
# 📊 مقيس 11 سبتمبر على 422 صفقة tactical_exit:
#    كان 6.0 و 62% من الصفقات ذروتها دونه فلا يتسلح الحارس اصلا.
#    الشريحة 3-6%: 132 صفقة ذروتها +4.35% وتُغلق عند +0.37% —
#    وصفر منها بلغت ذروتها بعد الاغلاق، اي كلها ارتدت قبله.
#    فالقفل عند 3% لا يقطع ربحا واحدا: +380 بدل +49.
PEAK_ARM_AT = 3.0
...
async def open_from_signal(sig: Signal, user_id: str = "system", amount: float = 100.0):
    ...
    # 📊 قياس 20 أغسطس: حصّة 5 لكل منصّة (=30) أعطت 168 صفقة بـ-208.6%
