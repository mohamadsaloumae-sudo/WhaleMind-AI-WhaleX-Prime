...
# 🛡️ سقف الخسارة القصوى — الوقف بالرافعة لا بالمسافة وحدها.
#    مقيس 12 سبتمبر على 164 صفقة MX في 7 ايام:
#      خسارة قصوى 15-20%  n=43 فوز 60% مجموع +42
#      خسارة قصوى 20-25%  n=29 فوز 65% مجموع +40
#      خسارة قصوى 25-30%  n=31 فوز 41% مجموع -64  ← الحد الفاصل
#      خسارة قصوى 30-45%  n=61 فوز 60% مجموع -25
#    و MX كله -8. بحد 25% يصير +82.
#    السبب: وقف 8% مع رافعة 5x يعني السماح بخسارة 40%، فالصفقة
#    تنهار قبل ان يمسكها اي حارس (BREW خسرت -16% في صفر دقيقة).
MAX_LOSS_PCT = 25.0


MIN_LEV = 5.0   # الحد الادنى للرافعة (السطر 318) — هو ما يحكم الخسارة


def _levels(price: float, direction: str, atr_pct: float) -> dict:
    sl_pct = max(2.0, min(MAX_SL_PCT, atr_pct * 2.0))
    # الرافعة تُشتق لاحقا من الوقف لكن حدها الادنى 5x، فوقف 8%
    # يعني خسارة 40%. نحد الوقف هنا بحيث لا تتجاوز MAX_LOSS_PCT.
    sl_pct = min(sl_pct, MAX_LOSS_PCT / MIN_LEV)
