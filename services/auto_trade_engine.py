...
async def execute_for_user_tracked(user_id: str, signal: dict) -> dict:
    ...
    try:
        ...
        if result.get("success"):
            ...
            # ℹ️ لا حاجة لربط الصفقة: المدير يُغلق عبر
            #    close_all_real_users(symbol, direction) لكل من فتحها.
            #    وmark_position_real غير موجودة في position_manager فتُسجّل
            #    خطأ مع كل فتح (10/ربع ساعة) بلا أثر.
        else:
            log.warning(
...
async def on_signal_approved(sig) -> None:
    ...
    #    أقلّ 45%. ويوزّع الحمل على باينانس بدل مئات الأوامر دفعةً.
    try:
        from services.hourly_cap import allow as _hc
        _ok, _why = _hc(sig.symbol)
        if not _ok:
            log.info("⏱️ %s %s مُؤجَّلة — %s",
                     sig.symbol, sig.direction, _why)
            return
    except Exception as _he:
        log.debug("السقف الساعيّ: %s", _he)
