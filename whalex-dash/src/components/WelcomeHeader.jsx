// 👋 ترحيب + علم الدولة + السوشيال + مشاركة الرقم
import { useEffect, useState } from "react";
import { useLang } from "../context/LangContext.jsx";
import { api } from "../lib/api.js";

const WA_CHANNEL = "https://whatsapp.com/channel/0029Vb8KTaF6RGJFztzS0Q3K";

// مفاتيح الدول الشائعة
const DIAL = {
  AE: "+971", SA: "+966", EG: "+20", KW: "+965", QA: "+974", BH: "+973", OM: "+968",
  JO: "+962", LB: "+961", IQ: "+964", SY: "+963", YE: "+967", PS: "+970", MA: "+212",
  DZ: "+213", TN: "+216", LY: "+218", SD: "+249", TR: "+90", US: "+1", GB: "+44",
  DE: "+49", FR: "+33", IN: "+91", PK: "+92", ID: "+62", MY: "+60", NG: "+234",
};

const ICONS = {
  telegram: (
    <svg viewBox="0 0 24 24" width="20" height="20" fill="#229ED9">
      <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.568 8.16c-.169 1.858-.896 6.728-.896 6.728-.169.896-.896 1.12-1.456.784l-2.688-1.96-1.288 1.232c-.169.169-.336.336-.672.336l.224-3.024 5.32-4.816c.224-.224-.056-.336-.336-.168l-6.552 4.144-2.856-.896c-.616-.224-.616-.616.112-.896l11.2-4.312c.504-.168.952.112.784.848z"/>
    </svg>
  ),
  youtube: (
    <svg viewBox="0 0 24 24" width="21" height="21" fill="#FF0000">
      <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
    </svg>
  ),
  x: (
    <svg viewBox="0 0 24 24" width="17" height="17" fill="#ffffff">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
    </svg>
  ),
  whatsapp: (
    <svg viewBox="0 0 24 24" width="21" height="21" fill="#25D366">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.71.306 1.263.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0 0 20.885 3.4"/>
    </svg>
  ),
  tiktok: (
    <svg viewBox="0 0 24 24" width="19" height="19" fill="#ffffff">
      <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/>
    </svg>
  ),
};

const SOCIALS = [
  { id: "telegram", label: "Telegram", url: "https://t.me/whaleXApp",                     bg: "rgba(34,158,217,0.14)" },
  { id: "youtube",  label: "YouTube",  url: "https://youtube.com/@whalemindhybridai",     bg: "rgba(255,0,0,0.12)" },
  { id: "x",        label: "X",        url: "https://x.com/Whale_Mind_AI",                bg: "rgba(255,255,255,0.08)" },
  { id: "tiktok",   label: "TikTok",   url: "https://tiktok.com/@whalemind.ai.bot",       bg: "rgba(255,255,255,0.08)" },
  { id: "whatsapp", label: "WhatsApp", url: WA_CHANNEL,                              bg: "rgba(37,211,102,0.13)" },
];

function uid() {
  try {
    const t = window.Telegram?.WebApp?.initDataUnsafe?.user?.id;
    if (t) return String(t);
  } catch { /* */ }
  let v = localStorage.getItem("wx_uid");
  if (!v) { v = "u" + Math.random().toString(36).slice(2, 10); localStorage.setItem("wx_uid", v); }
  return v;
}

function tgName() {
  try {
    const u = window.Telegram?.WebApp?.initDataUnsafe?.user;
    if (u) return [u.first_name, u.last_name].filter(Boolean).join(" ") || u.username || "";
  } catch { /* */ }
  return localStorage.getItem("wx_name") || "";
}

export default function WelcomeHeader() {
  const { lang } = useLang();
  const ar = lang === "ar";
  const [p, setP] = useState({});
  const [askPhone, setAskPhone] = useState(false);
  const [phone, setPhone] = useState("");
  const dial = DIAL[(p.country_code || "").toUpperCase()] || "+";

  async function savePhone() {
    const full = (dial === "+" ? "+" : dial) + phone;
    try {
      await api.post("/api/profile/phone", { user_id: "", phone: full });
      setP((x) => ({ ...x, phone: full }));
      setAskPhone(false);
    } catch { /* */ }
    window.open(WA_CHANNEL, "_blank");
  }

  useEffect(() => {
    api.post("/api/profile/track", { user_id: "", name: tgName() }).catch(() => {});
    api.get("/api/profile/me")
      .then((d) => { setP(d || {}); setAskPhone(!d?.phone); })
      .catch(() => {});
  }, []);



  const name = p.name || tgName() || "";

  return (
    <div style={{ marginBottom: 16 }}>
      {name && (
        <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 12 }}>
          <span style={{ fontSize: 17, fontWeight: 800, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {name}
          </span>
          {p.flag && <span style={{ fontSize: 21, lineHeight: 1 }}>{p.flag}</span>}
        </div>
      )}

      {askPhone && (
        <div style={{
          padding: 13, borderRadius: 13, marginBottom: 12,
          background: "rgba(37,211,102,0.09)", border: "1px solid rgba(37,211,102,0.24)",
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}>
            <svg viewBox="0 0 24 24" width="15" height="15" fill="#25D366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.71.306 1.263.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0 0 20.885 3.4"/></svg>
            <b style={{ fontSize: 12.5, color: "#25D366" }}>
              {ar ? "قناة واتساب — الإعلانات والعروض" : "WhatsApp channel — news & offers"}
            </b>
          </div>
          <div style={{ fontSize: 11, color: "var(--txt-3)", marginBottom: 9, lineHeight: 1.6 }}>
            {ar ? "أدخل رقمك ثم انضم للقناة بضغطة" : "Enter your number, then join with one tap"}
          </div>
          <div style={{ display: "flex", gap: 6, marginBottom: 8 }} dir="ltr">
            <span style={{
              flexShrink: 0, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)",
              borderRadius: 9, padding: "10px 11px", fontSize: 13, fontWeight: 700, color: "var(--txt-2)",
            }}>{dial}</span>
            <input
              value={phone}
              onChange={(e) => setPhone(e.target.value.replace(/[^0-9]/g, ""))}
              placeholder={ar ? "رقم الجوال" : "Mobile number"}
              inputMode="numeric"
              style={{
                flex: 1, minWidth: 0, background: "rgba(255,255,255,0.05)",
                border: "1px solid rgba(255,255,255,0.1)", borderRadius: 9,
                padding: "10px 12px", color: "inherit", fontSize: 13.5, outline: "none",
              }}
            />
          </div>
          <button
            onClick={savePhone}
            disabled={phone.length < 6}
            style={{
              width: "100%", background: "#25D366", color: "#04140a", border: "none",
              borderRadius: 10, padding: "11px", fontSize: 13.5, fontWeight: 700,
              cursor: "pointer", opacity: phone.length < 6 ? 0.45 : 1,
            }}
          >{ar ? "حفظ والانضمام للقناة" : "Save & join channel"}</button>
        </div>
      )}

      <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
        {SOCIALS.map((s) => (
          <a key={s.id} href={s.url} target="_blank" rel="noreferrer" title={s.label}
            style={{
              width: 42, height: 42, borderRadius: 13, display: "grid", placeItems: "center",
              background: s.bg, border: "1px solid rgba(255,255,255,0.09)",
              textDecoration: "none", transition: "transform .15s",
            }}>{ICONS[s.id]}</a>
        ))}
      </div>
    </div>
  );
}
