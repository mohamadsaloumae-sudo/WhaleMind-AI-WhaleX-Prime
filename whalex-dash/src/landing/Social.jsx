...
/** ⑫ تواصل معنا — أسفل المقدّمة */
export default function Social({ lang = "ar" }) {
  const ar = lang !== "en";
  return (
    <section style={{ padding: "6px 20px 30px", background: T.bg, textAlign: "center" }}>
      <div style={{ fontSize: 12.5, color: T.txt3, marginBottom: 16 }}>
        {ar ? "تابعنا على" : "Follow us on"}
      </div>
      <div style={{ display: "flex", justifyContent: "center", gap: 26 }}>
        {SOCIALS.map((s) => (
          <a key={s.id} href={s.url} target="_blank" rel="noreferrer" title={s.label}
             style={{ width: 26, height: 26, display: "flex", opacity: .8,
