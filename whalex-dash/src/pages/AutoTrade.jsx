      ...
      {mode === "spot" && (
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <input type="number" min="5" defaultValue={settings?.spot_trade_amount || 5}
            onBlur={(e) => {
              const bal = (balance?.usdt_total_spot ?? balance?.usdt_free) ?? 0;
          ...
          {spotErr ? (
            ...
            <div style={{ fontSize: 12, color: "var(--txt-3)", marginTop: 6 }}>
              {lang === "ar" ? `رصيدك يكفي ` : `Balance covers `}<b>{Math.max(0, Math.floor(((balance?.usdt_total_spot ?? balance?.usdt_free) ?? 0) / (settings?.spot_trade_amount || 5)))}</b>{lang === "ar" ? ` صفقة` : ` trades`}
            </div>
          )}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "14px 0 6px" }}>
            <span style={{ fontSize: 13, color: "var(--txt-2)" }}>{lang === "ar" ? "أوتو (كل الصفقات)" : "Auto (all signals)"}</span>
            <div onClick={() => saveSpot({ spot_max_positions: (settings?.spot_max_positions ?? 0) === 0 ? Math.max(1, Math.floor(((balance?.usdt_total_spot ?? balance?.usdt_free) ?? 0) / (settings?.spot_trade_amount || 5))) : 0 })}
              style={{ width: 52, height: 28, borderRadius: 14, cursor: "pointer", flexShrink: 0,
          ...
          {(settings?.spot_max_positions ?? 0) !== 0 && (
            <>
              ...
              <div style={{ fontSize: 11, color: "var(--txt-3)", marginTop: 4 }}>
                {lang === "ar" ? `الحد الأقصى بحسب رصيدك: ` : `Max by your balance: `}<b>{Math.max(1, Math.floor(((balance?.usdt_total_spot ?? balance?.usdt_free) ?? 0) / (settings?.spot_trade_amount || 5)))}</b>
              </div>
            </>
          )}
        </div>
