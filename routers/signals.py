...
def signals_history(market: str = "futures", user=Depends(get_current_user)):
    # 💰 تنفيذ المشترك أولا — صفحة "صفقاتي" تعرض ما نُفّذ له لا
    #    اشارات النظام. مقيس 17 سبتمبر: SYNUSDT نُفّذت بـ0.20469
    #    وبطاقة التفاصيل تعرض 0.19936 (سعر الاشارة)، فيرى المشترك
    #    سعرا لم يدخل به ونسبة لا تطابق حسابه.
    try:
        _uc = sqlite3.connect("file:/opt/whalex/db/whalex.db?mode=ro", uri=True)
        _uc.row_factory = sqlite3.Row
        if market == "spot":
            _q = ("SELECT symbol, entry, exit_price, qty, spend, pnl_pct, ts, "
                  "closed_ts, real_net, real_fee, exchange "
                  "FROM spot_positions_multi WHERE user_id=? AND status='closed' "
                  "AND (closed_ts > (strftime('%s','now','+4 hours','start of day','-4 hours')) "
                  "OR ts > (strftime('%s','now','+4 hours','start of day','-4 hours'))) "
                  "ORDER BY closed_ts DESC LIMIT 300")
            _r = [dict(x) for x in _uc.execute(_q, (user["sub"],))]
            _uc.close()
            if _r:
                _o = []
                for x in _r:
                    ...
                    _o.append({
                        ...
                        "grade": "A", "tier": "SP", "radar": "🪙 رادار السبوت",
                        "result": "win" if (_net or x["pnl_pct"] or 0) > 0 else "loss",
                        "pnl_pct": x["pnl_pct"],
                        "is_win": bool((_net if _net is not None else x["pnl_pct"] or 0) > 0),
                        "closed_at": x["closed_ts"], "opened_at": x["ts"],
                        "leverage": 1, "exchange": x["exchange"],
                        "net_usdt": _net, "commission": x["real_fee"],
                        "qty": x["qty"], "close_reason": "spot_exit",
                        "duration_min": round(((x["closed_ts"] or 0) - (x["ts"] or 0)) / 60, 1),
                    })
                return {"history": _o, "source": "live"}
        elif market == "futures":
            _q = ("SELECT symbol, direction, entry, exit_price, qty, leverage, "
                  "pnl_pct, net_usdt, real_net, commission, real_fee, opened_at, "
                  "closed_at, close_reason, exchange FROM user_trades "
            ...
            if _r:
                _o = []
                for x in _r:
                    _net = x["real_net"] if x["real_net"] is not None else x["net_usdt"]
                    _fee = x["real_fee"] if x["real_fee"] is not None else x["commission"]
                    _o.append({
                        "symbol": x["symbol"], "direction": x["direction"],
                        "entry": x["entry"], "exit_price": x["exit_price"],
                        "grade": "A", "tier": "LIVE", "radar": "⚡ تنفيذ حقيقيّ",
                        "result": "win" if (_net or 0) > 0 else "loss",
                        "pnl_pct": x["pnl_pct"], "is_win": bool((_net or 0) > 0),
                        "closed_at": x["closed_at"], "opened_at": x["opened_at"],
                        "leverage": x["leverage"], "exchange": x["exchange"],
                        "net_usdt": _net, "commission": _fee, "qty": x["qty"],
                        "close_reason": x["close_reason"],
                        "duration_min": round(((x["closed_at"] or 0) - (x["opened_at"] or 0)) / 60, 1),
                    })
                return {"history": _o, "source": "live"}
        _uc.close()
    except Exception as _le:
        log.debug("live history: %s", _le)
