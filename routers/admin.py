...
def user_detail(user_id: str, user=Depends(require_admin)):
    ...
    try:
        ...
        rows = [r[0] for r in con.execute(
            # 📊 المصدر الموحَّد
            __import__("services.stats_core", fromlist=["x"]).FUTURES_FILTER and "SELECT pnl_pct FROM training_signals WHERE " + __import__("services.stats_core", fromlist=["x"]).FUTURES_FILTER)]
        con.close()
    ...
    # 🗂️ اي سوق فعّله — فنفتح دفتره وحده في اللوحة
    try:
        from routers.admin_ledger import _enabled as _men
        out["markets_enabled"] = _men(user_id)
    except Exception:
        out["markets_enabled"] = []
    try:
